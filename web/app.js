...
function renderResults(data) {
  ...
  const droneCard=$("droneKpi").closest('article');
  const droneDetail=`<div class="segment-popover-title"><b>无人机充能与用量 / 日</b></div><div class="flow-ledger"><div><span>基础充能</span><strong>240 架</strong></div><div><span>发电站充能</span><strong>${signed((m.drones_recovery_potential_per_day||m.drones_per_day)-240,1)} 架</strong></div><div><span>总充能</span><strong>${number(m.drones_recovery_potential_per_day||m.drones_per_day)} 架</strong></div>${(m.drone_effect?.allocations||[]).map(item=>`<div><span>${escapeHtml(String(item.target||item.label).split('：')[0])}</span><strong>${number(item.drones_per_day)} 架</strong></div>`).join('')}<div><span>溢出</span><strong>${number(m.drone_overflow_lost_per_day||0)} 架</strong></div></div>`;
  droneCard.dataset.popover=String(++popoverSerial);popoverBodies.set(String(popoverSerial),droneDetail);droneCard.tabIndex=0;droneCard.setAttribute('role','button');droneCard.setAttribute('aria-label','查看每日无人机充能与投入用量');
  ...
  const reminders=(data.warnings||[]).filter(text=>/心情耗尽|不足以|无法|未找到/.test(text)).map(text=>text.includes('心情耗尽')?'部分干员会在下次上线前耗尽心情，模拟已计入影响。':text);
  $("warnings").innerHTML=[...new Set(reminders)].map(text=>`<div class="warning">${escapeHtml(text)}</div>`).join('');
  $("warnings").closest('details').hidden=!reminders.length;
  const coverage=data.catalog_mechanism_coverage;
  $("mechanismCoverage").textContent=coverage?`产出技能已适配 ${coverage.exact_count} / ${coverage.total_relevant} · 包含各精英阶段`:'';
  ...
  $("rooms").innerHTML = Object.entries(teams).map(([team,plan])=>{
    ...
    const teamSummary=staggered?'<p class="staggered-team-note">房间独立换班；每张设施卡按该房间实际在岗比例折算，展示完整轮换周期的日均贡献。</p>':`<div class="team-cycle-summary"><div><span>周期占比</span><strong>${(share*100).toFixed(1)}%</strong><small>${workHours[team]||cycleHours}h / ${cycleHours}h</small></div><div><span>对图表龙门币贡献</span><strong>${number(Math.round(BaseView.income(tm).net*share))}</strong><small>本队满日等效 ${number(Math.round(BaseView.income(tm).net))}</small></div><div><span>对图表经验贡献</span><strong>${number(Math.round((+tm.exp_per_day||0)*share))}</strong><small>本队满日等效 ${number(Math.round(+tm.exp_per_day||0))}</small></div><div><span>对图表赤金净流贡献</span><strong>${signed((+tm.gold_net_per_day||0)*share)}</strong><small>本队满日等效 ${signed(+tm.gold_net_per_day||0)}</small></div></div>`;
    ...
    const roomShare=staggered?(+(roomDurations[room.room]?.[team]||0)/(Object.values(roomDurations[room.room]||{}).reduce((sum,h)=>sum+(+h||0),0)||24)):share;
...
function renderGoldMonitor(m) {
  ...
  $("goldMonitor").innerHTML = `
    ...
    <div class="net-flow ${net < 0 ? "draining" : "growing"}"><span>库存速度</span><strong>${signed(net)} / 日</strong></div>
...
function renderRotation(rotation) {
  ...
  const handoverTimes=(rotation.handover_events||[]).filter(node=>node.time<(rotation.display_hours||rotation.cycle_hours)).length;
  $("rotationPattern").textContent=`${rotation.display_hours||rotation.cycle_hours}h 安排 · ${handoverTimes} 个换班时刻 · 每 ${rotation.collection_interval_hours}h 上线`;
  const cycle=rotation.display_hours||rotation.cycle_hours;
  ...
  const ticks=tickHours.map(hour=>`<span class="${hour===0?'axis-first':hour===cycle?'axis-last':''}" style="left:${hour/cycle*100}%">${hourLabel(hour)}</span>`).join('');
  const droneEvents=(rotation.production_curve?.drone_events||[]).filter(event=>event.minute/60<=cycle);
...
function resourceIcon(kind) { return `<img src="/assets/resources/${kind}.png" alt="" loading="lazy">`; }
const resourceOptions=[['lmd_net_after_shards_per_day','lmd','净收入'],['lmd_per_day','lmd','贸易收入'],['lmd_shard_cost_per_day','originium-shard','搓玉消耗'],['exp_per_day','battle-record','经验'],['gold_net_per_day','gold','赤金净流'],['gold_made_per_day','gold','赤金制造'],['gold_used_per_day','gold','赤金消耗'],['orundum_per_day','orundum','合成玉'],['drones_per_day','drone','无人机']];
let curveMetric='lmd_net_after_shards_per_day';
$("curveMetric").innerHTML=resourceOptions.map(([key,icon,label])=>`<button type="button" data-resource="${key}" aria-pressed="${key===curveMetric}">${resourceIcon(icon)}<span>${label}</span></button>`).join('');
function droneEventInfo(event,target) {
  return `<div class="segment-popover-title"><b>${hourLabel(event.minute/60)} · 无人机</b><span>投入前 ${signed(event.drone_inventory_before,1)} 架 · 本次共用 ${signed(event.drones_spent,1)} 架</span></div><div class="flow-ledger">${(target?[target]:event.targets||[]).map(item=>`<div><span>${escapeHtml(String(item.target||item.label).split('：')[0])}</span><strong>${signed(item.drones,1)} 架</strong></div><p>${escapeHtml(item.label)} · 加速 ${signed(item.drones*3,1)} 分钟</p>`).join('')}</div>`;
}
...
function renderYieldCurve(curve) {
  if (!curve?.points?.length) {
    $("yieldCurve").innerHTML='<p class="side-note">生成轮班后显示产出曲线。</p>';
    return;
  }
  const key=curveMetric;
  const label=curveLabels[key]||key;
  const displayHours=state.lastResult?.rotation?.display_hours||curve.hours||24;
  const points=curve.points.filter(point=>point.minute<=displayHours*60);
  const final=+points.at(-1).cumulative[key]||0;
  const rates=points.map(point=>+point.rates_per_hour[key]||0);
  const transitions=`${(state.lastResult?.rotation?.handover_events||[]).filter(event=>event.time<displayHours).length} 个时刻，见轮换运行图`;
  ...
  $("yieldCurve").innerHTML=`<div class="curve-summary"><div><span>${key==="drones_per_day"?`${displayHours}h 末库存`:`${displayHours}h 累计`}</span><strong>${signed(final)} ${escapeHtml(label)}</strong></div><div><span>${key==="drones_per_day"?"恢复速率范围":"小时速率范围"}</span><strong>${signed(Math.min(...rates))} ～ ${signed(Math.max(...rates))}</strong><small>每分钟 ${signed(Math.min(...rates)/60,3)} ～ ${signed(Math.max(...rates)/60,3)}</small></div><div><span>实际换班时刻</span><strong>${escapeHtml(transitions)}</strong></div><div><span>排程口径</span><strong>${escapeHtml(weighting||"房间独立")}</strong><small>${cycle?`平均工时仅作摘要；曲线按房间逐段积分`:`直接使用单班日产`}</small></div></div><div id="yieldChartCanvas" class="echart yield-echart" role="img" aria-label="${escapeHtml(label)}累计与实时速率"></div><p class="side-note">${escapeHtml(curve.note)}</p>`;
  if (!window.echarts) return;
  state.charts.yield?.dispose();
  const colors=chartColors(), cumulativeLabel=key==="drones_per_day"?"当前库存":`累计${label}`;
  const droneData=(curve.drone_events||[]).filter(event=>event.minute<=displayHours*60).map(event=>{
  ...
  state.charts.yield.setOption({
    ...
    tooltip:{trigger:"axis",backgroundColor:colors.surface,borderColor:colors.line,textStyle:{color:colors.text},formatter(items){const hour=items[0]?.axisValue??0;const rows=items.map(item=>item.data?.event?`${item.marker}无人机投入：<b>${number(item.data.event.drones_spent)}</b> 架<br>${(item.data.event.targets||[]).map(target=>`${escapeHtml(target.label)}：${number(target.drones)} 架（加速 ${number(target.drones*3)} 分钟）`).join("<br>")}`:`${item.marker}${item.seriesName}：<b>${number(Math.round(item.value[1]*1000)/1000)}</b>${item.seriesIndex===1?" /h":""}`);return `<b>${hour}h</b><br>${rows.join("<br>")}`;}},
    xAxis:{type:"value",min:0,max:displayHours,interval:Math.max(2,displayHours/12),axisLabel:{color:colors.muted,formatter:"{value}h"},axisLine:{lineStyle:{color:colors.line}},splitLine:{lineStyle:{color:colors.line,opacity:.55}}},
...
function positionOperatorPopover(){
  if(!popoverTarget)return;
  const panel=$("operatorPopover"),rect=popoverTarget.getBoundingClientRect();
...
const rosterManager=createRosterManager({getCatalog:()=>state.catalog,getRoster:()=>state.roster,
  avatar:operatorAvatar,eliteIcon,skillIcon,escape:escapeHtml,
